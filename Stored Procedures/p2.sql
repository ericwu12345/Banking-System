...
language sql
  ...
  if(verifyAccNumExists(srcAccNum)) {
    exec sql select balance into :xfer_account_balance from p3.account where number = :src_cid; 
    if(xfer_account_balance < trans_amt) {
  ...
  if(verifyAccNumExists(destAccNum)) {
